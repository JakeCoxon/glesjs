...
_gl._texImage2DLong = function(target,level,internalFormat,width,height,border,format,type,pixels) {
	...
	console.log(getGLConstant(_gl, target), getGLConstant(_gl, level), getGLConstant(_gl, internalFormat),
			width, height, border, getGLConstant(_gl, format), getGLConstant(_gl, type), pixels);
...
function getGLConstant(ctx, value) {
...
document.createElement = function(nodeName) {
	if (nodeName.toUpperCase() == "CANVAS") { return new _canvasnode(); }
...
_2dcontext.prototype.getImageData = function(x,y,w,h) {
	...
	// ret = [];
	// for (var i=0; i<w*h*4; i++) ret.push(0xffaa00);
